...
function readJsonFiles() {
  return readdir(`${workPath}/${jsonPath}`).then((files: string[]) => {
    let promises: Promise<any>[] = [];
    files.forEach((lan: string) => {
      promises.push(readLanFiles(lan).then(() => writeJsonFile(lan)));
    });
...
function writeJsonFile(lan: string) {
  let jsonPath = `${workPath}/${destPath}/exml.${lan}.json`;
  return readFile(jsonPath, "utf-8")
    ...
    .then((fileStr: string) => {
      ...
      return writeFile(jsonPath, JSON.stringify(data)).then(() => {
        Log.appendLine(`更新文件：${workPath}/${destPath}/exml.${lan}.json`);
      });
    })
    .catch(() => {
      Log.appendLine(`更新文件失败：${workPath}/${destPath}/exml.${lan}.json`);
    });
}
